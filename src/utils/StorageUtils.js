...
export default class StorageUtils extends Component {
    ...
    static _getStorage() {
        if (storage === undefined) {
            storage = new Storage({
                ...
                //有了这个sync方法，以后再调用storage.load时，如果本地并没有存储相应的user，那么会自动触发storage.sync.user去远程取回数据并无缝返回。
                sync: {
                    // 这个sync文件是要你自己写的
                }
            });
    ...
    static _load(key, callBack, errorCallBack) {
        this._load3(key, null, null, callBack, errorCallBack);
    }


    static _load3(key, params, someFlag, callBack, errorCallBack) {
